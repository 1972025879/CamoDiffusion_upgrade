...
class Trainer(object):
    ...
    def save(self, epoch, max_to_keep=10):
        ...
        # 只保留标准格式的 checkpoint 文件（不包含 'last'），避免 'model-x-last.pt' 文件名解析错误
        standard_ckpt_files = [f for f in ckpt_files if not f.endswith('-last.pt')]
        ckpt_files = sorted(standard_ckpt_files, key=lambda x: int(x.split('-')[-1].split('.')[0]))
