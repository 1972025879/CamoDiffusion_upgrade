...
class PyramidVisionTransformerImpr(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        if cache_mask is None:
            cache_mask = {'c1': True, 'c2': True, 'c3': True, 'c4': True} # 默认缓存全部
        self.cache_mask = cache_mask
        self.enable_caching = False
        self.cache = {'c1': None, 'c2': None, 'c3': None, 'c4': None} # 扩展缓存字典
        self.cache_counter = 0
        self.cache_T = cache_T

    def _forward_full(self, x, timesteps, cond_img):
    ...
    def _forward_with_caching(self, x, timesteps, cond_img):
        ...
        # Stage 1 (c1)
        should_cache_c1 = self.cache_mask.get('c1', False)
        if should_cache_c1 and self.cache['c1'] is not None:
            print("加载c1缓存")
            c1 = self.cache['c1']
            x = c1
        else:
...
# Main net class
class net(nn.Module):
    def __init__(self, class_num=2, mask_chans=0, backbone_cache_T=2, backbone_cache_mask=None, **kwargs):
        super(net, self).__init__()
        self.class_num = class_num
        # Hardcoded cache_mask including c1
        backbone_cache_mask = {'c1': False, 'c2': False, 'c3': False, 'c4': False} # 全部缓存
        # Pass cache_T and cache_mask to backbone
