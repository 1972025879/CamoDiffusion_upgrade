...
from denoising_diffusion_pytorch.simple_diffusion import ResnetBlock, LinearAttention # 修改导入路径
...
class OverlapPatchEmbed(nn.Module):
    ...
    def forward(self, x, mask=None): #Image xt
        x = self.proj(x)
...
class PyramidVisionTransformerImpr(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
                 ...
                 cache_T=2, cache_mask=None):  # <<< 新增 cache_T 和 cache_mask
        super().__init__()
        ...
        if cache_mask is None:
            cache_mask = {'c2': True, 'c3': True, 'c4': True}
        self.cache_mask = cache_mask
        self.enable_caching = False
        self.cache = {'c2': None, 'c3': None, 'c4': None}
        self.cache_counter = 0
        self.cache_T = cache_T

    def _forward_full(self, x, timesteps, cond_img): #cond_img是Image x是噪声中间状态的xt
    ...
    def _forward_with_caching(self, x, timesteps, cond_img):
        B = x.shape[0]
        outs = []

        # Stage 1: always compute
        time_token = self.time_embed[0](timestep_embedding(timesteps, self.embed_dims[0]))
        time_token = time_token.unsqueeze(dim=1)
        x, H, W = self.patch_embed1(cond_img, x)
        x = torch.cat([time_token, x], dim=1)
        ...
        # Stage 2 (c2)
        should_cache_c2 = self.cache_mask.get('c2', False)
        if should_cache_c2 and self.cache['c2'] is not None:
            print("加载c2缓存")
            c2 = self.cache['c2']
            x = c2
        else:
        ...
        # Stage 3 (c3)
        should_cache_c3 = self.cache_mask.get('c3', False)
        if should_cache_c3 and self.cache['c3'] is not None:
            c3 = self.cache['c3']
            print("加载c3缓存")
        else:
            print("计算c3缓存")
            ...
            time_token = time_token.unsqueeze(dim=1)
            x_input = x
            x, H, W = self.patch_embed3(x_input)
            x = torch.cat([time_token, x], dim=1)
        ...
        # Stage 4 (c4)
        should_cache_c4 = self.cache_mask.get('c4', False)
        if should_cache_c4 and self.cache['c4'] is not None:
            c4 = self.cache['c4']
            print("加载c4缓存")
        else:
            print("计算c4缓存")
            time_token = self.time_embed[3](timestep_embedding(timesteps, self.embed_dims[3]))
            time_token = time_token.unsqueeze(dim=1)
            x_input = c3  # c4 的输入是 c3
            x, H, W = self.patch_embed4(x_input)
            x = torch.cat([time_token, x], dim=1)
        ...
        # Update cache counter and refresh
        self.cache_counter += 1
        if self.cache_counter >= self.cache_T:
            # Only clear caches that are masked to be cached
            for key in self.cache_mask:
                if self.cache_mask[key]:
                    self.cache[key] = None
            self.cache_counter = 0
    ...
    def forward(self, x, timesteps, cond_img):
        if self.enable_caching:
            return self._forward_with_caching(x, timesteps, cond_img)
        else:
            return self._forward_full(x, timesteps, cond_img)
...
class pvt_v2_b4_m(PyramidVisionTransformerImpr):
    def __init__(self, cache_T=2, cache_mask=None, **kwargs):  # <<< 透传 cache_T 和 cache_mask
        super(pvt_v2_b4_m, self).__init__(
            patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[4, 4, 4, 4],
            qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 8, 27, 3], sr_ratios=[8, 4, 2, 1],
            drop_rate=0.0, drop_path_rate=0.1, cache_T=cache_T, cache_mask=cache_mask, **kwargs)
...
# Utility functions and Decoder (unchanged)
def resize(input,
           size=None,
           scale_factor=None,
           mode='nearest',
...
class Decoder(nn.Module):
    def __init__(self, dims, dim, class_num=2, mask_chans=1):
        ...
        self.fuse=ConvModule(in_channels=embedding_dim * 2, out_channels=embedding_dim, kernel_size=1,
                    norm_cfg=dict(type='BN', requires_grad=True))
        self.down_2=nn.Sequential(
             # Stage 2: H/4 -> H/8 (stride=2)
            ConvModule(in_channels=embedding_dim, out_channels=embedding_dim*2, kernel_size=3, padding=1, stride=2,
                       norm_cfg=dict(type='BN', requires_grad=True)),
            resnet_block(embedding_dim*2, embedding_dim*2, time_emb_dim=self.time_embed_dim),
            ConvModule(in_channels=embedding_dim*2, out_channels=embedding_dim*2, kernel_size=3, padding=1,
                       norm_cfg=dict(type='BN', requires_grad=True)),
        )
        self.down_3=nn.Sequential(# Stage 3: H/8 -> H/16 (stride=2)
            ConvModule(in_channels=embedding_dim * 2, out_channels=embedding_dim * 4, kernel_size=3, padding=1, stride=2,
                    norm_cfg=dict(type='BN', requires_grad=True)),
            resnet_block(embedding_dim * 4, embedding_dim * 4, time_emb_dim=self.time_embed_dim),
        ...
        self.up_1 = nn.Sequential(
            Upsample(embedding_dim*4, embedding_dim*2, factor=2),
            ConvModule(in_channels=embedding_dim*2, out_channels=embedding_dim*2, kernel_size=3, padding=1,
                       norm_cfg=dict(type='BN', requires_grad=True)),
        ...
        self.up_2=nn.Sequential(
            ConvModule(in_channels=embedding_dim * 4, out_channels=embedding_dim*2, kernel_size=1,
                    norm_cfg=dict(type='BN', requires_grad=True)),#fuse跳跃连接
            Upsample(embedding_dim*2, embedding_dim, factor=2),
            ConvModule(in_channels=embedding_dim, out_channels=embedding_dim, kernel_size=3, padding=1,
                       norm_cfg=dict(type='BN', requires_grad=True)),
        )
    ...
    def forward(self, inputs, timesteps, x):
        ...
        L34 = self.linear_fuse34(torch.cat([_c4, _c3], dim=1)) # 计算 L34
        ...
        # print("PRED:",x.shape)
        # --- L34 计算逻辑 ---
        return x, c1, c2, c3, c4 # 返回计算出的 L34，以便在 net 中缓存
        # --- L34 计算逻辑结束 ---
...
class net(nn.Module):
    def __init__(self, class_num=2, mask_chans=0, **kwargs):
        ...
        # Hardcoded cache_mask and cache_T
        backbone_cache_mask = {'c2': True, 'c3': True, 'c4': True}
        backbone_cache_T = 2
        self.backbone = pvt_v2_b4_m(
            in_chans=3, 
            mask_chans=mask_chans,
            cache_T=backbone_cache_T,
            cache_mask=backbone_cache_mask  # <<< 透传
        )
    ...
    def forward(self, x, timesteps, cond_img):
        self.backbone.enable_caching = self.is_sampling
        features = self.backbone(x, timesteps, cond_img)
        output, _, _, _, _= self.decode_head(features, timesteps, x)
        return output
    ...
    # @torch.inference_mode() # 移除或注释掉，以便在采样时可以修改 is_sampling
