...
from model.CamoFormer_decoder import MSA_module,Conv_Block
...
class Decoder(nn.Module):
    def __init__(self, dims, dim, class_num=2, mask_chans=1):
       ...
       # 原始路径 - 保持原有流程
        ...
        # MSA路径 - 新增路径
        from model.CamoFormer_decoder import MSA_module, Conv_Block
        self.conv_block = Conv_Block(embedding_dim)
        
        # MSA模块，添加残差连接
        self.MSA5 = MSA_module(dim=embedding_dim)
        self.MSA4 = MSA_module(dim=embedding_dim) 
        self.MSA3 = MSA_module(dim=embedding_dim)
        self.MSA2 = MSA_module(dim=embedding_dim)
        
        # MSA路径的侧边卷积
        self.side_conv1 = nn.Conv2d(embedding_dim, embedding_dim, kernel_size=3, stride=1, padding=1)
        self.side_conv2 = nn.Conv2d(embedding_dim, embedding_dim, kernel_size=3, stride=1, padding=1)
        self.side_conv3 = nn.Conv2d(embedding_dim, embedding_dim, kernel_size=3, stride=1, padding=1)
        self.side_conv4 = nn.Conv2d(embedding_dim, embedding_dim, kernel_size=3, stride=1, padding=1)
        
        # MSA路径的融合模块
        self.fuse1 = nn.Sequential(
            nn.Conv2d(embedding_dim*2, embedding_dim, kernel_size=3, stride=1, padding=1, bias=False),
    ...
    def forward(self, inputs, timesteps, x):
        ...
        for blk in self.down:
         ...
         ############## 原始路径 ##############
        n, _, h, w = c4.shape
        _c4_orig = self.linear_c4(c4).permute(0, 2, 1).reshape(n, -1, c4.shape[2], c4.shape[3])
        _c4_orig = resize(_c4_orig, size=c1.size()[2:], mode='bilinear', align_corners=False)
        _c3_orig = self.linear_c3(c3).permute(0, 2, 1).reshape(n, -1, c3.shape[2], c3.shape[3])
        _c3_orig = resize(_c3_orig, size=c1.size()[2:], mode='bilinear', align_corners=False)
        _c2_orig = self.linear_c2(c2).permute(0, 2, 1).reshape(n, -1, c2.shape[2], c2.shape[3])
        _c2_orig = resize(_c2_orig, size=c1.size()[2:], mode='bilinear', align_corners=False)
        _c1_orig = self.linear_c1(c1).permute(0, 2, 1).reshape(n, -1, c1.shape[2], c1.shape[3])
        
        # 原始融合
        L34_orig = self.linear_fuse34(torch.cat([_c4_orig, _c3_orig], dim=1)) 
        L2_orig = self.linear_fuse2(torch.cat([L34_orig, _c2_orig], dim=1))
        orig_feat = self.linear_fuse1(torch.cat([L2_orig, _c1_orig], dim=1))
        ############## 双路径融合 ##############
        fused_feat = self.path_fusion(torch.cat([orig_feat, msa_feat], dim=1))
        print("fused_feat:",fused_feat.shape)
        # 最终预测
        x = torch.cat([fused_feat, x], dim=1)
        for blk in self.up:
            if isinstance(blk, ResnetBlock):
                x = blk(x, t)
            else:
...
class net(nn.Module):
    ...
    def sample_unet(self, x, timesteps, cond_img, **kwargs):
        # 修改sample_unet方法以接受return_multi_scale参数
        output = self.forward(x, timesteps, cond_img)
        
        if kwargs.get('return_multi_scale', False):
            return output
        else:
